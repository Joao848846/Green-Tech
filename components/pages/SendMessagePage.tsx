...
const SendMessagePage: React.FC = () => {
  ...
  const [messageResponse, setMessageResponse] =
    useState<SendTextMessageResponse | null>(null);
  ...
  const fetchOpenInstances = useCallback(async () => {
    ...
    try {
      ...
      if (!allInstances || allInstances.length === 0) {
        toast("API não retornou nenhuma instância conectada.", {
          duration: 4000,
        });
        setInstances([]); // ensure state is empty
        return;
      }

      // Filter for open instances, trimming status string for robustness
      const openInstances = allInstances.filter(
        (inst) =>
          typeof inst.connectionStatus === "string" &&
          inst.connectionStatus.trim().toLowerCase() === "open"
      );
      ...
      if (openInstances.length > 0) {
        ...
        // Optional: toast.success(`${openInstances.length} instância(s) 'open' encontrada(s).`);
      } else {
        // This message is shown if allInstances was populated, but none were 'open' after filtering
        toast.error(
          "Nenhuma instância com status 'open' foi encontrada entre as conexões retornadas. Verifique o status das conexões na página 'Gerenciar Instâncias'.",
          { duration: 5000 }
        );
      }
    } catch (err) {
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      try {
        ...
        // setRecipientNumber(''); // Clear fields on success - commented out to allow easy resend
        // setMessageText('');
      } catch (err) {
        const apiErr = err as ApiError;
    ...
    <div className="space-y-6">
      <header className="pb-4 border-b border-futuristic-primary/20">
        ...
        <p className="mt-1 text-md text-futuristic-text-secondary">
          Selecione uma instância, insira o destinatário e a mensagem.
        </p>
      </header>
      ...
      <Card>
        {isLoadingInstances ? (
          <Spinner />
        ) : instances.length === 0 ? ( // This 'instances' state now only holds 'open' instances
          <p className="text-center text-futuristic-text-secondary py-4">
            Nenhuma instância com status "open" disponível para envio. Verifique
            a página "Gerenciar Instâncias" para o status das conexões ou se a
            API retornou alguma instância.
          </p>
        ) : (
